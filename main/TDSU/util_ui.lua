...
-- Draws a small rect at the ui position.
...
function createSlider(tb, key, title, valueText, min, max, w, h, fs)
    ...
    do UiPush()
        ...
        -- if math.abs(InputValue("mousedx")) > 0 then
		-- 	PlaySound(LoadSound("MOD/snd/UI/UI_SliderTick.ogg"))
		-- end
...
function DrawCameraSprite(path, pos, l, w, r, g, b, a, dt)
...
function UiProgressBar(val, max, w, h, r, g, b, critical)
	UiPush()
	local w = w or 300
	local h = h or 30
	local pad = 6
	...
	UiAlign("left top")
	UiColor(0, 0, 0, 0.5)
	UiImageBox("ui/common/box-solid-10.png", w, h, borderWidth, borderHeight)
	if t > 0 then
		UiTranslate(pad / 2, pad / 2)
		w = (w - pad) * t
		h = h - pad
		borderWidth = math.min(w / 2, 6)
		local critPercent = math.min(t / critical, 1)
		UiColor(1 - (1 - r) * critPercent, g * critPercent, b * critPercent, 1)
		UiImageBox("ui/common/box-solid-6.png", w, h, borderWidth, borderHeight)
	end
	UiPop()
end

function UiProgressBarTitled(title, val, max, w, h, r, g, b, critical, empty_warning)
UiPush()
	...
	UiPush()
		...
		--! Fix
		-- if val / max <= empty_warning then
		-- 	UiColor(1,0,0, 1/3+GetInterval(2/3)* 2/3)
		-- end
...
function UiProgressBarTitledA(title, val, max, w, h, r, g, b, a)
	UiPush()
		...
		UiPush()
			...
			-- local critPercent = math.min(t / critical, 1)
			local critPercent = 1
			UiColor(1 - (1 - r) * critPercent, g * critPercent, b * critPercent, a)
