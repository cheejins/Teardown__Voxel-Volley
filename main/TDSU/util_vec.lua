...
GetCrosshairWorldPos = function(rejectBodies, dist)

    local crosshairTr = GetCrosshairCameraTr()
    if rejectBodies then
        RejectAllBodies(rejectBodies)
    end
    local crosshairHit, crosshairHitPos = RaycastFromTransform(crosshairTr, dist or 300)
...
function GetPlayerTrackingPos(playerPos, source_pos, vel_len)

    playerPos = playerPos or VecAdd(GetPlayerTransform().pos, Vec(0,0.7,0))

    local vel = GetPlayerVelocity()
    local scale = VecDist(source_pos, playerPos) / vel_len

    local aimPos = VecAdd(playerPos, VecScale(vel, scale))
...
--- Get the transform at the gun pos requried to shoot a moving vehicle.
function GetPlayerTrackingPos(playerPos, source_pos, vel_len)
...
--- Get the transform at the gun pos requried to shoot a moving vehicle.
function GetBodyTrackingPos(body, source_pos, vel_len)

    local tr = GetBodyTransform(body)
    local pos_com = TransformToParentPoint(tr, GetBodyCenterOfMass(body))

    local vel = GetBodyVelocity(body)
    local scale = VecDist(source_pos, pos_com) / vel_len

    local aimPos = VecAdd(GetBodyTransform(body).pos, VecScale(vel, scale))
...
--- Get the transform at the gun pos requried to shoot a moving vehicle.
function GetVehicleTrackingPos(vehicle, source_pos, vel_len)
    return GetBodyTrackingPos(GetVehicleBody(vehicle), source_pos, vel_len)
end


function IsInfrontOfTransform(tr, pos) return TransformToLocalPoint(tr, pos)[3] < 0 end



-- Source: OpenAi
function RayIntersectsAABB(ray_origin, ray_direction, aabb_min, aabb_max)
	local tmin = -math.huge
	local tmax = math.huge

	for i = 1, 3 do
		if math.abs(ray_direction[i]) < 1e-6 then
			if ray_origin[i] < aabb_min[i] or ray_origin[i] > aabb_max[i] then
				return false
			end
		else
			local inv_dir = 1.0 / ray_direction[i]
			local t1 = (aabb_min[i] - ray_origin[i]) * inv_dir
			local t2 = (aabb_max[i] - ray_origin[i]) * inv_dir
...
function LineIntersectsAABB(line_start, line_end, box_min, box_max)

    local tmin, tmax = 0, 1

    for i = 1, 3 do
        if line_start[i] ~= line_end[i] then
            local t1 = (box_min[i] - line_start[i]) / (line_end[i] - line_start[i])
            local t2 = (box_max[i] - line_start[i]) / (line_end[i] - line_start[i])
        ...
        elseif line_start[i] < box_min[i] or line_start[i] > box_max[i] then
            return nil
        end
    end
    ...
    local intersection = {}
    for i = 1, 3 do
        intersection[i] = line_start[i] + (line_end[i] - line_start[i]) * tmin
    end
...
-- Recursive function to find the closest pair of 3D points (divide and conquer algo)
function closest_points(points)
    local n = #points

    -- Base case: return infinity if there are less than 2 points
    if n < 2 then
        return math.huge
    end
    ...
    local d1 = closest_points(left)
    local d2 = closest_points(right)
    local d = math.min(d1, d2)

    -- Find the closest pair of points that cross the middle
    local strip = {}
    for i = 1, n do
        if math.abs(points[i].x - points[mid].x) < d then
            table.insert(strip, points[i])
        end
    end
    table.sort(strip, function(a, b) return a.y < b.y end)
    local k = #strip
    for i = 1, k do
        for j = i + 1, k do
            ...
            local dist = VecDist(strip[i], strip[j])
            if dist < d then
                d = dist
            end
        end
...
function GetBodySpeed(body)
    return TransformToLocalVec(GetBodyTransform(body), GetBodyVelocity(body))[3] * -1
end
